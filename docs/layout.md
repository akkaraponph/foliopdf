...
```go
page := doc.AddPage(presspdf.A4.Landscape())  // 297 x 210
```

Custom size (values in PDF points, 1 pt = 1/72 inch):

```go
page := doc.AddPage(presspdf.PageSize{WidthPt: 400, HeightPt: 600})
```
...
cols := presspdf.NewColumnLayout(doc, page, 3, 5)  // 3 columns, 5mm gutter
cols.Begin()
