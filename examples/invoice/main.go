...
//     embedded via presspdf.Document.RegisterImage (which only accepts JPEG)
...
func main() {
	...
	logo, err := loadLogoAsJPEG("assets/logo-presspdf.png")
	if err != nil {
	...
	w.page = doc.AddPage(presspdf.A4)
...
func (w *writer) drawNotes() {
	...
	notes := "กรุณาชำระเงินภายใน 14 วันนับจากวันที่ในใบแจ้งหนี้ โดยโอนเข้าบัญชี " +
		...
		"เมื่อชำระเงินแล้วโปรดส่งหลักฐานการโอนมาที่ billing@presspdf.studio / " +
		"Please remit payment within 14 days of the invoice date and email " +
		"the transfer slip to billing@presspdf.studio."
...
// it can be embedded via presspdf.Document.RegisterImage (which currently
// only accepts JPEG).
